  ...
  if [[ -z "${token}" ]]; then
    ...
    while true; do
      read -r -s -p "  Enter DISCORD_TOKEN: " token </dev/tty 2>/dev/null || {
        ...
        warn "  curl -fsSL ${REPO_URL%%.git}/raw/main/scripts/install-vprod.sh | sudo env DISCORD_TOKEN='your-token' bash"
        exit 1
      }
      echo
...
write_config() {
  ...
  {
    printf '{\n'
    printf '  "name": "Vantage | Production",\n'
...
install_motd() {
  ...
  # These are chmod -x so PAM skips them; the files are kept so that future
  # Ubuntu updates do not re-enable them automatically.
  ...
  # ── Write our MOTD script ─────────────────────────────────────────────────
  {
    ...
    printf '# Installed by install-vprod.sh — delete this file to disable.\n'
